...
* Fit a HAL with CV-selected penalty and an undersmoothed HAL.  
...
# 1. Introduction 
The Highly Adaptive Lasso (HAL) is a non-parametric machine learning algorithm that "creates efficient estimators of realistic statistical models of pathwise differentiable parameters. Furthermore, these models provide asymptotically normal estimators for non-pathwise-differentiable parameters, which can enable investigators to use HAL-based substitution estimators and the delta method rather than rely on parametric models." [**Highly Adaptive LASSO: Machine Learning That Provides Valid Nonparametric Inference in Realistic Models**] 

This github page is to provide example code for straightforward uses of HAL. The R scripts used to generate the figures in the accompanying manuscript are provided in the `figures/` folder of this repository (see `figures/README.md`), for transparency and reproducibility.

# 2. How to Fit HAL
In this section, we demonstrate the functions for fitting HAL.
...
First, identify the outcome variable and save it as a numeric vector. In this example, we are interested in weight-for-height z-score (`whz`) as the outcome, which is continuous. 
```{r}
...
We can use the `smoothness_orders` argument to enforce smoothness on the functional form of the HAL fit.   
...
hal_fit_firstorder <- fit_hal(X = X, Y = Y)
```


### 2.2.2 Undersmoothed HAL
Three steps to fit a undersmoothed HAL:  
1. We need to fit a cv-hal with the argument `return_x_basis = TRUE` by calling the `fit_hal` function.  
...
3. Then we fit the undersmoothed HAL by calling `fit_hal` function again with:  
  ...
  - and other arguments the same as the initial fitting of cv-hal.   
...
We will use the previously obtained fitted undersmoothed HAL object, `hal_u_fit`, to get the predicted whz value for each individual within the dataset.
```{r}
...
# 4. Obtaining Estimates and Inferences
...
Suppose we are interested in the average weight-for-height z-score (`whz`) if all of the people in Bangladesh had the WSH interventions (i.e. the counterfactual outcome mean): $\mathbb{E}_P(Y_1)$.
...
# 5. Dose Response Curve Example with a Simulated Dataset
## 5.1 Fit HAL
## 5.2 Dose response curve estimation
## 5.3 Confidence intervals


# 6. Reproducing the manuscript figures

The `figures/` folder of this repository contains stand-alone R scripts that
reproduce the two simulation figures in the accompanying manuscript. They are
provided for transparency and reproducibility, and build on prior work that is
credited in `figures/README.md`. Each script is run from inside the `figures/`
folder and writes its own `.png` and `.eps` output there.

## 6.1 Figure 1 — HAL at several sectional variation norms

`fig1_hal.R` fits HAL (using zero-order indicator basis functions and `glmnet`)
to data generated from a smooth, multi-frequency target function,
$Y = \sin(1.2 X) + 0.7\sin(2.7 X) + \varepsilon$. It then draws three fits at
different **sectional variation norms** — an over-regularized (low-norm) fit,
the cross-validation–selected fit, and an under-regularized (high-norm) fit —
together with the true function and the data. The figure illustrates how the
variation-norm penalty controls the bias–variance trade-off: too small a norm
oversmooths, too large a norm overfits, and cross-validation lands in between.

```sh
# from the figures/ folder:
Rscript fig1_hal.R     # writes fig1_hal.png and fig1_hal.eps
```

This script depends only on `glmnet` and base R (plus `hal_helpers.R` and
`hal_style.R`, which are sourced automatically).

## 6.2 Figure 2 — bootstrap CI width and the lambda plateau

`fig2_density_width.R` reproduces the confidence-interval-width plot used to
choose the HAL penalty $\lambda$ for nonparametric bootstrap inference. For a
simulated multimodal density, it computes the Wald interval width across a grid
of $\lambda$ values and plots width against $\lambda$ (log scale). As $\lambda$
decreases below the cross-validation choice (i.e. as the fit is undersmoothed),
the width increases and then **stabilizes on a plateau**; that plateau marks the
$\lambda$ value used for the final bootstrap interval.

```sh
# from the figures/ folder:
Rscript fig2_density_width.R   # writes fig2_density_width.png/.eps
```

This script requires the `TMLEbootstrap` package. See the dependency note in
`figures/README.md` (current `hal9001` needs observation weights passed via
`fit_control`).
