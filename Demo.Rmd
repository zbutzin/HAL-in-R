...
# 5. Reproducing the manuscript figures
...
provided for transparency and reproducibility, and build on prior work
(Benkeser, HAL simulation code; Phillips, Coyle, and Hejazi, *hal9001*; Cai and
van der Laan, 2020) that is credited in `figures/README.md`. Each script is run
from inside the `figures/` folder and writes its own `.png` and `.eps` output
there.

## Figure 1. HAL can fit complex functions.
...
different sectional variation norms: an over-regularized (low-norm) fit, the
cross-validation-selected fit, and an under-regularized (high-norm) fit,
together with the true function and the data. The figure shows how the
variation-norm penalty controls the bias-variance trade-off. A norm that is too
small oversmooths the data, a norm that is too large overfits it, and
cross-validation selects a norm in between.
...
## Figure 2. Optimizing bias-variance tradeoff.
